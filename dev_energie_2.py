...
def magnetic(particle_name, magnetic_field):
    ...
    print(result)
    if result:
        write('allpix_conf/detector.conf',10,f'position = {result}mm 0cm 21cm')
    return 0

def main():

    for particle in ['pi+','kaon+','mu+']:
        write('allpix_conf/detector.conf',10,f'position = -40mm 0cm 21cm')


        for i in np.arange(100,1050,50):
            write('allpix_conf/simple_detector.conf',18,f'source_energy = {i}MeV')
